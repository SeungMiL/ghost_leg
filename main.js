const Yl = {
  ...
  create: function () {
    ...
    for (let i = 0; i < this.nNum; i++) {
      ...
      this.arrGoDiv[i].innerHTML =
        '<input type="button" value="GO" onClick="Yl.start(' + i + ')">';
    ...
    for (let i = 0; i < this.nNum * 7; i++) {
      let nLen = this.arrHorDiv.length;
  ...
  sRndTop: "",
  getRndTop: function () {
    let nRnd = parseInt(Math.random() * (this.nHeight - 100)) + 150;
  ...
  moveStart: function (sXy, pno, nSx, nSy) {
    let nLen = this.arrMovDiv.length;
    ...
    for (let i = 0; i < this.arrHorDiv.length; i++) {
      let nx = parseInt(this.arrHorDiv[i].style.left);
      let ny = parseInt(this.arrHorDiv[i].style.top);
      let nw = parseInt(this.arrHorDiv[i].style.width);
      let nh = parseInt(this.arrHorDiv[i].style.height);
  ...
  move: function (no, pno, nSx, nSy, nEx, nEy) {
    let nx = parseInt(this.arrMovDiv[no].style.left);
    let ny = parseInt(this.arrMovDiv[no].style.top);
    let nw = parseInt(this.arrMovDiv[no].style.width);
    let nh = parseInt(this.arrMovDiv[no].style.height);
    ...
    if (bIng) {
      ...
      if (nEy < this.nHeight + 100) {
        ...
        for (let i = 0; i < this.arrVerDiv.length; i++) {
          if (nEx == parseInt(this.arrVerDiv[i].style.left)) {
            this.arrBotDiv[i].innerHTML =
              "<b>" + (pno + 1) + "</b><br>" + this.arrBotDiv[i].innerHTML;
...
addEventListener('keydown',
    (e) => {
        if(e.keyCode == 71 && e.shiftKey && e.altKey){
